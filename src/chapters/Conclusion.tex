...
In one of my discussions with a historian, she told that, in this model, ``document nodes can be considered as \emph{Emics} and person nodes \emph{Etics} concepts''\cite{headlandEmicsEticsInsider1990}.
...
Almost no work has been done on the handling of uncertainty directly in network models\cite{adarManagingUncertaintySocial, schulzProbabilisticGraphLayout2017}, even though it would allow to ground results in a more rational and real vision of the textual data (at the cost of an increasing complexity).
