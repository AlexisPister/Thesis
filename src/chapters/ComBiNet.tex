...
In the previous chapter \autoref{ch:hsna-process-and-network-modeling}, I showed \rev{that \modelplural constitute a good modeling choice for historical documents in regard} of \traceability, \reality, and \simplicity properties.
...
\paperbox{This chapter is an extended version of an article currently being published in the journal Computer Graphics Forum (CGF)\cite{pister:hal-03927450} and a poster presented at the conference EuroVis 2022\cite{pisterVisualQueriesBipartite2022}. It was a joint work with my advisors Christophe Prieur and Jean-Daniel Fekete. I developed the interface and led the discussions, evaluation, and writing of the paper.}
...
I validate \combinet with the description of four real-world use cases showing the system can be used to answer socio-historical questions while reflecting on the annotation/encoding process, and a formative usability study demonstrating the system can be used smoothly by social historians.
...
Social scientists have for a long time described local structures of social networks with the aim of revealing interesting behaviors\cite{hollandLocalStructureSocial1976}.
...
Several scripting languages, such as \project{R}~\cite{Rstat} and \project{Python}~\cite{Python} have been extended to support the exploration of social networks using specialized libraries such as \project{igraph}~\cite{igraph} and \project{NetworkX}~\cite{networkx}, and provide functions for graph pattern matching.
...
To lower the complexity barrier of their usage, several visual graph query systems such as \project{QUOGGLES}\cite{holleisQUOGGLESQueryGraphs2005} and \project{VIGOR} been developed, which show the query that has been written in a visual form, enabling analysts to reflect on their constructed queries and explore the results more easily.
Other systems such as \project{GRAPHITE}~\cite{chauGRAPHITEVisualQuery2008}, \project{Intuinet}\footnote{See \url{https://intuinet.fr}}, and \project{VERTIGo}~\cite{cuencaVERTIGoVisualPlatform2021} hide the query language and allow users to directly construct their queries visually with node-link diagrams and interaction.
...
%Other systems, such as \project{VIGOR}~\cite{pientaVIGORInteractiveVisual2018}, only visualize the query after it has been written by the user with a scripting language (here Cypher) and do not allow direct interaction of the visual representation of the query.
...
\begin{enumerate}
    ...
    The corpus is made of family trees referring to several document/event types: birth and death certificates, marriage acts, military mobilizations, and census reports. The roles are different for each event type and consist of \textit{children, father, mother} for the birth events, \textit{deceased} for the death event, \textit{spouse} and \textit{witnesses} for the marriages, and \textit{family members} for the census events.
    \begin{footnotesize}
    \begin{description}
...
%6659 PERSONS. 1381 MARRIAGE_ACTS.
    ...
    The historian also has access to other documents mentioning the prices people have to pay to get their migration requests approved.
     \begin{footnotesize}
    \begin{description}
...
Nodes' labels are displayed (names of persons and ids of documents by default) on the canvas with an occlusion-free mechanism that hides nodes with a low degree when two or more nodes' labels overlap.
...
The \textit{selection} mode allows dragging the nodes in the panel, while the \textit{addition} and \textit{deletion} modes allow the following actions:
\begin{description}
...
%who can share the results of their queries easily or use them in another software or tool, according to the \traceability principle.
...
Similarly, different boolean comparison operations are available by hovering their respective buttons (\autoref{fig:combinet-overall} top of V9), such as the intersection, union, and differences between the two filters. % It allows to rapidly change filters to find interesting persons or group of persons in a exploratory process.
...
%    \caption{Distribution of two document attributes and one global attribute for the documents and signatories of Torino (A), Torino's surroundings (B), and the whole graph. (top).}\label{fig:attributeComparison}
\end{figure}
...
Afterward, we answered question 10, \ie is there a significant difference in the migrations between the 18th and 19th centuries.
...
We can clearly see that people do not move at all in the 18th century, while in the 19th century even if the majority of people stayed in the same place from their birth to their death, \rev{many individuals moved of departments.}
...
I describe in this fourth use case how \name can be used to answer questions about theses defended in France between 2016 and 2022.
Indeed, some sociological datasets made of documents can also be well modeled as \modelplural, like theses dissertations: a thesis is a document with specific attributes such as the subject, the doctoral school, the domain, the university, and the date of defense, and mention several peoples who are socially connected through the thesis defense with different roles: author (\textit{auteur} in french), director(s) (\textit{directeur}), reviewers (\textit{rapporteur}), and jury president (\textit{président de jury}).
%We discussed with sociologists who were interested in this type of data for explorations purposes.
I present here an exploration of the data using \name.
A first look at the network measures tells us that 896 theses have been defended in sociology in France between 2016 and 2021 in France, with 2453 persons included in the defenses (see \autoref{fig:combinet-thesis-exploration} bottom).
...
We are now interested in characterizing the theses occurring in this pattern, by their geolocations.
...
By selecting the Sankey view option, we can investigate if this pattern occurs between the thesis defended in different regions or if it occurs mainly in the same ones.
...
% \combinet hence gives an answer to \qtwo through a 
