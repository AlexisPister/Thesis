...
Many visualization techniques and softwares have thus been developed since the beginnings of \sna, but most popular tools are usually not designed for historians specifically, meaning that they do not regard on the provenance and process leading to the network, and focus on analysis aspects only.
Moreover, they typically enforce simple network models without proposing exploration mechanisms, beyond allowing to look at the network structure and computed measures.
...
If these computer-supported methods are now widely used in history\cite{kerschbaumerPowerNetworksProspects2015, petzCombiningNetworkResearch2022}, they attracted many criticism from the start---some of which are still relevant.
...
Most used visual software for \sna such as Gephi\cite{Gephi}, Pajek\cite{pajek}, NodeXl\cite{NodeXL}, Tulip\cite{auberTULIP2017}, or Ucinet\cite{ucinet} are based on this representation, and allow an exploration of the data with the help of basic interaction mechanisms and the computation of network measures.
...
\autoref{fig:keim-va} illustrates the schematic process of VA: the coupling of visualization and data mining models operated by the user through interaction lead to the generation of knowledge ``extracted'' from the data.
...
\begin{figure}[!ht]
    ...
    \caption{Node-link diagram of a medieval social network of peasants, produced with a force-directed layout, commonly used in \sna softwares. \rev{The picture is note very informative and only reveal a semblance of community structure.} Image from \cite{bouletBatchKernelSOM2008}.}
