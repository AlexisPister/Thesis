...
%\iffalse
%Unipartite (co-occurrence and multiplex) networks are still the norm in the literature. However, we believe that if this type of model can be useful for SNA, they often bring distortion and ambiguity to historical datasets, which are complex. Cristofoli argues that using a bipartite network allows modeling the social relationships with a better alignment with the original sources and points to two drawbacks of using unipartite projections\cite{cristofoliAuxSourcesGrands2008}: the information about the specific events is lost, meaning that historians cannot make the connection between a link and the event/document it was created from. Furthermore, several projections exist for the same network, meaning that social scientists must choose which projection to use.
