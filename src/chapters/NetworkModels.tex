...
We describe in this chapter the HSNA workflow followed by social historians, to shed light on their process and summarize recurring pitfalls to identify how VA could help them in this process.
...
Most HSNA practitioners report on their findings concerning the network they constructed from their sources, but few highlight their process which led to these conclusions from the raw historical documents.
Similarly, VA tools always focus on the analysis part, once the network have been constructed, without helping historians in the previous steps.
...
To study relational social structures where individuals influence each others such as families, companies, and institutions, historians can rely on \hsma by modeling the social relationships between a set of entities---usually individuals---into a network.
...
Moreover, historical documents are often complex and the annotation, and modeling process can be done in many different ways, concerning what to annotate from the documents\cite{lemercierBackSourcesPracticing2021} and how to model the annotation in a network\cite{cristofoliAuxSourcesGrands2008}.
...
\subsection{Historian methodology}
...
With the development of social and quantitative history, historians now have a panoply of methods to exhaustively extract quantitative data from their sources and analyzing it to ground their results in verifiable claims.
Many historians criticized this computational aspect of history\cite{lepetitHistoireQuantitativeDeux1989, fogelLimitsQuantitativeMethods1975}, pointing that it would lead to errors and missing the core content of historical sources.
However, using quantitative approaches and formalisms are not exclusive with having a deep understanding of the documents and their context, nor building a narrative on top of their quantitative analysis.
...
However, formalisms have to be used wisely and with a critical vision of the documents and their context, to not fall into simplifications, anachronisms, and errors which are pertinent critics of quantitative history\cite{lemercierQuantitativeMethodsHumanities2019, lemercierBackSourcesPracticing2021}.
...
The level of plausibility of a claim increase or decrease depending on if the different evidence point to similar results or not.
...
Similarly, historians often work on small populations or specific individuals---as it is the case with microhistory studies\cite{ginzburgMicrohistoire1981}---which can results in complications for generalization.
Only after studying several similar individuals or groups, historians are able to generalize, and to point to exceptions.
For example, by comparing several jewish commercial communities in Europe during the first half of the 18th century, Trivellato have been able to generalize to what is common to those groups (they have been trading between them and with outer ethnic groups) and what is specific to each (such as their business strategies) \cite{trivellatoThereFutureItalian2011}.
...
Several textbooks describe and explain to social scientists and students who do not have formal computer science training in what consist these methods (statistical regression, Chi-squared test, network analysis, etc.) and how to practically use those with softwares and programming language \cite{floudIntroductionQuantitativeMethods2013}.
However, the process leading from the sources to the numeric artefacts (a table, a network, a timeline) have not been described thoroughly in the literature, especially with concrete examples, and is often not presented in scientific publications of concrete use cases.
Yet, the process leading from the documents to analyzable data requires social historians to make several annotation, encoding and modeling decision, concerning \emph{what} to extract from the source and \emph{how} to encode it.
This process is tedious and requires data acquisition, annotation, encoding, modification with continuous back and forth between the different steps \cite{alkadi2022}.
%The process to generate a clean quantitative or network dataset from historical sources is difficult and requires several data acquisition, annotation, and cleaning steps.
%Social analysts are not always trained on how to do these steps effectivity, which can lead to errors, inconsistencies, and mismatches between the chosen data models and the historical questions~\cite{alkadi2022}.
This is a critical step as it can lead to simplifications, anachronism, distortion, or data which do not allow to answer original or new hypotheses \cite{karila-cohenNouvellesCuisinesHistoire2018, lemercierQuantitativeMethodsHumanities2019}.
Lemercier and al. give guidelines on how to encode information from historical documents to prevent introducing bias, by having a critical view on the documents \cite{lemercierBackSourcesPracticing2021}.
They emphasize the importance of the input phase of research and advise to copy the first documents by hand while characterizing them the most exhaustive and factual way, without imposing categorization.
This explorative step let historians familiarize with the content of the document, leading to a better view of what to encode to answer their research questions and sometimes to the formulation of new hypotheses.
For example, in their project on the social and geographical trajectories of Jews in Lubartów\cite{zakrzewski1932PopulationRegister2020}, a village in Poland, the team noted the mean of writing inside the register documents (pen, pencil, ink, etc.) they were studying.
This information  allowed them to conclude that the inscription ``expelled'' in pencil was probably added during the second word war by germans to denote exported jews in the extermination camps.
...
Dufournaud present her quantitative and network workflow when studying the economical role of women during the 16th and 17th centuries in the city of Nantes, that she splits in three steps: data collection, data processing, and data analysis \cite{dufournaudCommentRendreVisible2018}.
...
The choice of the network model has a major impact on the future analysis and \textbf{may add bias if chosen loosely (P6)}, such as the creation of network structural artefacts when using network projections\cite{cristofoliAuxSourcesGrands2008}.
...
Currently, most digital historical projects use unipartite networks (simple, co-occurrence, and multiplex) that are simple and allow answering specific questions, but they do not capture all the complexity of the documents, resulting in simplifications and distorsions of the structural patterns.
For example, modeling only co-occurrences of persons in documents remove the variety of social relationships these mentions can refer to\cite{lemercier12FormalNetwork2015}.
...
The lack of simplicity therefore result in difficulties for visualization and analysis, especially for social scientists.
...
Most historical work do not provide details how they constructed their final network, even though it is a complicated and tedious process which can result in many biases and distortions if not done carefully\cite{alkadi2022}.
